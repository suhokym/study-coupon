...
public enum CouponIssueRequestCode {
    ...
    INVALID_COUPON_ISSUE_QUENTITY(3);
    ...
    public static CouponIssueRequestCode find(String code) {
        ...
        if(codeVal == 2) return DUPLICATED_COUPON_ISSUE;
        if(codeVal == 3) return INVALID_COUPON_ISSUE_QUENTITY;
    ...
    public static void checkRequestResult(CouponIssueRequestCode code){
        if(code == INVALID_COUPON_ISSUE_QUENTITY){
